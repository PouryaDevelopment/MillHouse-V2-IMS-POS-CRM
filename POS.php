...
<img class="h-6 w-6"  src="./UI/notification.png">
    ...
    <div class="text-black  p-1  
      ...
      <div class="flex overflow-hidden h-6 ml-12 justify-end tracking-wide  " >
        <button class="mx-3 border-b-2 border-amber-300 transition ease-in duration-200 transform hover:-translate-y-1 active:translate-y-0" style="  font-family: 'Bebas Neue'; " onclick="POSpage()">
          <span>Point Of Sale</span>
        </button>

        <button class="mx-3 transition ease-in duration-200 transform hover:-translate-y-1 active:translate-y-0" style="  font-family: 'Bebas Neue'; " onclick="recieptpage()">
          <span>Reciept</span>
        </button>
  ...
  <div id="content_container" class="h-full grid grid-rows-3 grid-flow-col gap-4 overflow-y-auto overflow-x-hidden text-center">
    <?php

if (isset($_GET['message'])) {
    $reqmessage = $_GET['message'];
    if ($reqmessage == "success") {
         echo '<div id="ALERT" class="alert  alert-dismissible fade show col-span-8 h-6 text-center  bg-gradient-to-r from-amber-200/25 via-amber-100/25 to-amber-200/25  shadow-inner rounded-bl-full border-stone-500/25 mt-0  p-0 font-mono font-extralight text-stone-500 "  role="alert">
  Thank you!
 ...
 ?>
<?php include('functions/POS/application.php'); ?>
